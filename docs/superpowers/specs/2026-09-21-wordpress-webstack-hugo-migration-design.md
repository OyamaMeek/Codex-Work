...
- 其中 3 条已发布导航没有 `favorites` 分类关系；为完整保留数据，将其置于末尾的“其他”分组。
...
- 使用 `sqlglot` 解析 SQL 备份；不手写字符串解析器。
...
- 转换测试使用真实 SQL 数据，检查 44 个链接记录均被读取、12 个原始分类和末尾“其他”分组均被生成。
